    ...
    public class DownloadController : Controller
    {
        ...
        public async Task<IActionResult> DownloadAsync(string indexerId, string path, string jackett_apikey, string file)
        {
            try
            {
                ...
                try
             {
                    // Check if the torrent appears to be valid (starts with 'd')
                ...
                {
                    // invalid torrent → do not attempt to parse
                    sortedDownloadBytes = downloadBytes;
                }
             }
                catch (Exception e)
             {
                    _logger.Warn($"Bencode parse failed, returning raw torrent. Reason: {e.Message}");
    
                    // fallback: raw torrent
                    sortedDownloadBytes = downloadBytes;
             }
